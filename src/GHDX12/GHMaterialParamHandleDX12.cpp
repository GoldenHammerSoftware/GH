...
void GHMaterialParamHandleDX12::setValue(HandleType type, const void* value)
{
	for (unsigned int i = 0; i < mHandles.size(); ++i)
	{
		...
		{
			GHMaterialShaderInfoDX12::TextureList& textureList = mHandles[i].first->mTextures[mHandles[i].second->mCBType];
