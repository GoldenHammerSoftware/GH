...
void main()
{
 ...
 float angle = mod(Time*2.0, 2.0*pi);
    
  float eps = 0.5;
  float a = step(pi - eps, angle);  //if angle is above our lower threshold around 180
  float b = step(pi + eps, angle); //if angle is above our upper threshold around 180
  float c = step(0.5, a - a*b); //if angle is within our threshold around 180
    
  float x = step(eps, angle); //if angle is above our upper threshold around 360
  float y = step(pi*2.0 - eps, angle); //if angle is above our lower threshold around 360
  float z = 1.0 - (y - x); // if angle is within our threshold around 360  
